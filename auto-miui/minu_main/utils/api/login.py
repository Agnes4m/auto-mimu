...
class Login:
    ...
    async def login(
        ...
        try:
            ...
            if (
                self.cookies != {}
                and await BaseSign(self.cookies, self.user_agent).check_daily_tasks(
                    nolog=True
                )
                != []
            ):
            ...
            if api_data.success:
                ...
                check_url = await self.qr_login(bot)
    ...
    async def get_cookies_by_passtk(
        ...
        try:
            ...
            response = await get(
                "https://api.vip.miui.com/page/login", params=params, headers=headers
            )
    ...
    async def qr_login(self, bot: Bot) -> Tuple[str, bytes]:
        """二维码登录"""
        headers = {
        ...
        check_url = data["lp"]
        img = generate_qrcode(login_url)
        await bot.send(
            "本程序会记录你的登录状态，请你确保信任该机器人",
            "温馨提示：本程序出于学习目的，如造成账号封禁，本程序制作者恕不负责，请确保自己了解后果的情况下使用",
            img,
        )
        return check_url
